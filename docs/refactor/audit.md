...
|---|---|
| 🔴 Критические баги | 5 |
| 🔴 Безопасность (critical/high) | 8 |
| 🟡 Средние баги | 12 |
| 🟡 Производительность | 10 |
| 🟢 Code quality / dead code | 15+ |
| 💰 Монетизация | **$0 revenue при готовой premium-функциональности** |
...
|---|---|---|---|
| SEC-1 | **Захардкоженный Bot Token** | `bot/config.py:17` | Токен бота лежит в исходном коде: `7471812936:AAFoji...`. Кто угодно с доступом к репо контролирует бота. **Нужно ротировать токен через BotFather немедленно.** |
| SEC-2 | **Захардкоженный Bot Token** | `backend/config.py:44` | Тот же токен дублируется в бэкенд-конфиге как default. |
| SEC-3 | **Нет аутентификации на API** | Все роутеры | ВСЕ 12 роутеров принимают `user_id`/`telegram_id` как параметр без проверки. Любой HTTP-клиент может читать/менять/удалять данные любого пользователя. |
| SEC-4 | **Захардкоженный admin secret** | `backend/routers/grammar.py:293` | `os.getenv("ADMIN_SECRET", "mluv-seed-2026")` — если переменная не задана, секрет — в коде. Плюс передаётся как query param → логируется в access logs. |

### 🟡 HIGH

| # | Проблема | Файл | Описание |
|---|---|---|---|
| SEC-5 | **Сессии в памяти** | `backend/routers/web_auth.py:60` | `sessions: dict` хранится в RAM → теряется при рестарте. Нет лимита → DoS через флуд авторизаций. |
| SEC-6 | **JWT в localStorage** | `frontend/lib/auth-store.ts:36` | Токен в localStorage уязвим к XSS. Нужны httpOnly cookies. |
| SEC-7 | **DELETE без владения** | `backend/routers/words.py:174` | `DELETE /{word_id}` удаляет любое слово по ID без проверки принадлежности. |
| SEC-8 | **Push без авторизации** | `frontend/lib/push-notifications.ts:167` | `saveSubscriptionToServer` использует `fetch` вместо `apiClient` → запросы идут без токена. |

### 🟢 MEDIUM

| # | Проблема | Описание |
|---|---|---|
| SEC-9 | `datetime.utcnow()` deprecated | Python 3.12+ — использовать `datetime.now(timezone.utc)` |
| SEC-10 | Session token в query string | `GET /me` принимает токен как query param → попадает в логи и browser history |
| SEC-11 | Admin secret в query string | Оба admin endpoint `/admin/seed` и `/admin/send-notifications` |
| SEC-12 | Logout без проверки владения | `POST /logout` принимает любой токен без проверки |
...
| Проблема | Где |
|---|---|
| User response dict повторяется 3 раза | `web_auth.py` (строки 148, 206, 351) |
...
- Вынести user response в Pydantic response model
...
### In-memory хранилища (утечки памяти)
| Dict | Файл | Проблема |
|---|---|---|
| `sessions` | `web_auth.py` | Не очищается, нет TTL |
| `onboarding_data` | `bot/handlers/start.py` | Не очищается при незавершённом онбординге |
| `_corrections_cache` | `bot/handlers/voice.py` | Чистится только при новом сообщении |
| `_active_games` | `backend/services/game_service.py` | Не шарится между воркерами, нет TTL |
| `_active_scenarios` | `backend/services/scenario_service.py` | Не шарится между воркерами, нет TTL |

**Решение:** Перевести всё на Redis с TTL.
...
| # | Проблема | Файл |
|---|---|---|
| FE-1 | **Токен не сохраняется при логине** (см. BUG-1) | `login/page.tsx` |
| FE-2 | `useOptimisticMutations` — `user_mistakes` маппится на `string[]` вместо `Mistake[]` | `hooks/useOptimisticMutations.ts:87` |
| FE-3 | `telegramId=0` если user=null → запрос с несуществующим ID | `practice/page.tsx:60` |
| FE-4 | 401 interceptor вызывает logout на **каждый** 401, включая login | `api-client.ts:48` |
| FE-5 | `hasSeenWelcomeVideo()` вызывается при рендере (не в useEffect) → hydration mismatch | `dashboard/page.tsx:53` |

### Dead code
| Что | Файл |
|---|---|
| `useRequireAuth()` — никогда не используется | `auth-store.ts:78` |
| `useAuth()` — никогда не используется | `auth-store.ts:71` |
| `validateTelegramAuth()` — никогда не вызывается | `telegram-auth.ts:54` |
| `initTelegramAuth()` — никогда не используется | `telegram-auth.ts:26` |
| `applyTheme()` — никогда не используется | `theme-store.ts:36` |
| `AnalyticsData`, `WeeklyStats`, `DailyActivity` типы | `types.ts:105` |
| Импорты `dynamic`, `Skeleton`, `Trophy`, `Star` | layout.tsx, learn/page.tsx |
...
| # | Проблема | Файл |
|---|---|---|
| BOT-1 | `onboarding_data` dict растёт неограниченно | `handlers/start.py:21` |
| BOT-2 | `_corrections_cache` dict растёт неограниченно | `handlers/voice.py:34` |
...
### 🔴 Сейчас (1-3 дня)
1. **Ротировать Bot Token** через BotFather (захардкожен в коде)
...
6. Убрать hardcoded admin secret → в env var без fallback

### 🟡 Неделя 1-2
7. Добавить лимит сообщений (5/день)
8. Реализовать Telegram Stars payments
9. Перевести sessions → Redis
10. Добавить API authentication middleware (проверка session token)
...
23. Перевести in-memory state (games, scenarios) в Redis
