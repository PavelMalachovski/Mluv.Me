...
### 08.02.2026 - Команда /clear_history для удаления истории переписки ✅

**Проблема:** Кнопка "Restart" на каждом сообщении была избыточной и могла случайно сбросить контекст. Нужна была более мощная функция для полного удаления истории.

**Решение:**
- ❌ Удалена кнопка "Restart" из голосовых сообщений
- ✅ Добавлена команда `/clear_history` с подтверждением
- ✅ Команда удаляет ВСЮ историю переписки (необратимо!)
- ✅ Статистика и сохранённые слова остаются

**Изменённые файлы:**
- **Bot:**
  - `bot/handlers/voice.py` - удалена кнопка и callback handler
  - `bot/handlers/commands.py` - добавлена команда `/clear_history`
  - `bot/keyboards/settings.py` - добавлена клавиатура подтверждения
  - `bot/localization/cs.py` - добавлены тексты на чешском
  - `bot/services/api_client.py` - добавлен метод `delete_conversation_history()`
- **Backend:**
  - `backend/routers/messages.py` - новый роутер для управления сообщениями
  - `backend/main.py` - регистрация нового роутера
  - Endpoint: `DELETE /api/v1/messages/{telegram_id}/history`

**Преимущества:**
- ✅ Более осознанное действие (требует подтверждения)
- ✅ Полное удаление истории для "чистого старта"
- ✅ Меньше случайных нажатий
- ✅ Чистый интерфейс сообщений (только кнопка "Text")
