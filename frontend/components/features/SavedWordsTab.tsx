...
export function SavedWordsTab({ telegramId }: SavedWordsTabProps) {
  ...
  const { data: savedWords, isLoading } = useQuery<SavedWord[]>({
    queryKey: ["saved-words", telegramId],
    queryFn: () => apiClient.getSavedWords(telegramId),
    enabled: !!telegramId,
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["saved-words", telegramId] })
    },
  })
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-3">
                    <h3 className="text-lg font-semibold text-foreground">{word.word_czech}</h3>
                    {word.phonetics && (
                  ...
                  <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
                    <span>Opakováno {word.times_reviewed}×</span>
                    <span>•</span>
                    <span>Přidáno {new Date(word.created_at).toLocaleDateString("cs-CZ")}</span>
                  </div>
                </div>
