...
class Settings(BaseSettings):
    ...
    frontend_port: int = Field(default=3000, description="Port for Next.js frontend")
    ...
    redis_cache_ttl_user: int = Field(
        default=3600, description="User data cache TTL in seconds (1 hour)"
    )

    redis_cache_ttl_stats: int = Field(
        default=900, description="Stats cache TTL in seconds (15 minutes)"
    )

    redis_cache_ttl_openai: int = Field(
        default=86400, description="OpenAI response cache TTL in seconds (24 hours)"
    )

    # API Configuration
    max_voice_duration: int = Field(
        default=60, description="Maximum voice message duration in seconds"
    )

    rate_limit_per_minute: int = Field(
        default=20, description="Maximum API requests per user per minute"
    )
    ...
    whisper_model: str = Field(
        default="whisper-1", description="Whisper model for speech-to-text"
    )

    tts_model: str = Field(default="tts-1", description="TTS model for text-to-speech")

    tts_voice: str = Field(
        default="alloy", description="Voice for Honzik (alloy or onyx)"
    )
